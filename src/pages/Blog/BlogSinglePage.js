...
const BlogSinglePage = ({ match, history }) => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      const postRaw = await firestore
        .collection('posts')
        .where('post_alias', '==', postId)
        .get();
      let postData;
      ...
      <Row>
        ...
        >
          ...
          {postInfo.createdate ? (
            <i>
              {new Date(
                postInfo.createdate.seconds * 1000
              ).toLocaleDateString()}{' '}
            </i>
          ) : null}
          <small className='text-muted'>
            Category: {categories[postInfo.category]}
          </small>
  ...
  return <div>{postLoaded ? postBody : <Loader />}</div>;
};
